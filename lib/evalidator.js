...
function EValidator(options) {
    ...
    var extractValue_ = function (obj, attr, isFunction) {
        if (attr in obj)
            return obj[attr];
    ...
    var execRule_ = function (attr, rule, errorMap, done) {
        var isFunction = (typeof rule === 'function');
        var value = extractValue_(obj_, attr, isFunction),
            errors = [],
        ...
        if (_.isObject(rule) && rule.validator) {
            ...
            var validatorName = rule.validator,
                ...
                allowEmpty = rule.allowEmpty || !(validatorName == 'notEmpty');
